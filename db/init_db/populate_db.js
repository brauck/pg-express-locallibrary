...
function bookCreate(title, author, summary, isbn, genres) {
  const query = {text: '', values: []};

  if (genres) {
    query.text = 'INSERT INTO models.book (title, author, summary, isbn, genre) VALUES($1, $2, $3, $4, $5)';
    query.values =  [title, author, summary, isbn, genres];
    return query;
  }
  if (!genres) {
...
(async () => {
  ...
  try {
    ...
    await client.query(bookCreate('Test Book 1', 5, 'Summary of test book 1', 'ISBN111111', [genres[0],genres[1]]));
